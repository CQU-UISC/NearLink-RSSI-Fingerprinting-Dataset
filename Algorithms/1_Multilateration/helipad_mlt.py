...
def find_distance(TNRSSIdbm):
    RSSI0dbm = -45.33    # Constant C (reference RSSI in dBm)
    n = 1.925     # Pathloss exponent
    dist_buffer = []
...
def multilateracao(APsLoc, TNRSSIdbm):
    ...
    cond_init = np.array([8, 8])  # Adjusted initial guess to center of new area
...
# Scenario dimensions
largura = 16    # Updated to match new AP coordinates
comprimento = 16  # Updated to match new AP coordinates
...
plt.xticks(np.arange(0, 16.1, 1))  # Adjusted for new width
plt.yticks(np.arange(0, 16.1, 1))  # Adjusted for new length
plt.xlabel('x [m]')
plt.ylabel('y [m]')
plt.legend()
plt.grid()
