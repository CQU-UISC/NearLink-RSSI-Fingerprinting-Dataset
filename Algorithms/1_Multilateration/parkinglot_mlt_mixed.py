...
TECH_PARAMS = {
    'wifi': {'RSSI0dbm': -46.77, 'n': 1.798},
    'ble': {'RSSI0dbm': -44.41, 'n': 1.863},
    'sle': {'RSSI0dbm': -44.05, 'n': 1.905}
}

# Function to estimate distance from RSSI for a specific technology
def find_distance(TNRSSIdbm, tech, num_anchors=8):
...
def multilateracao(APsLoc, TNRSSIdbm, tech_columns):
    # tech_columns: dict mapping tech to RSSI column indices, e.g., {'wifi': [0,1,2,3,4,5,6,7], 'ble': [8,9,10,11,12,13,14,15]}
    dist_final = []
    ap_indices = []
    ...
    cond_init = np.array([11.25, 14.25])
...
], index=('APS1', 'APS2', 'APS3', 'APS4', 'APS5', 'APS6', 'APS7', 'APS8'), columns=['x', 'y'])

# Process each mixed dataset
datasets = {
    'wifi_ble': {'file': 'wifi_ble_dataset.csv', 'tech_columns': {'wifi': list(range(0, 8)), 'ble': list(range(8, 16))}},
    'wifi_sle': {'file': 'wifi_sle_dataset.csv', 'tech_columns': {'wifi': list(range(0, 8)), 'sle': list(range(8, 16))}},
    'ble_sle': {'file': 'ble_sle_dataset.csv', 'tech_columns': {'ble': list(range(0, 8)), 'sle': list(range(8, 16))}},
    'wifi_sle_ble': {'file': 'wifi_sle_ble_dataset.csv', 'tech_columns': {'wifi': list(range(0, 8)), 'sle': list(range(8, 16)), 'ble': list(range(16, 24))}}
}
